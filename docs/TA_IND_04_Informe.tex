...
un único volumen de \num{500000} registros. Por esta razón, no es posible
identificar de forma independiente los tres parámetros del modelo utilizando
solamente la evidencia actual.

En consecuencia, no se presenta un valor numérico definitivo para $n^{*}$.
Utilizar las configuraciones de uno, dos y cuatro executors como si
representaran distintos valores de $n$ no sería correcto, ya que en esas
pruebas cambia $N$, pero el volumen de entrada permanece constante.

Para determinar el umbral sería necesario repetir T1 con varios tamaños de
entrada y mantener controladas las demás condiciones del experimento. Gulino
et al. estudian modelos para predecir el tiempo de ejecución de aplicaciones
Spark considerando características del volumen procesado y los recursos
asignados \cite{gulino2020}. De manera relacionada, Lattuada et al. estudian
la asignación de recursos en aplicaciones Spark mediante modelos orientados a
cumplir objetivos de rendimiento \cite{lattuada2020}.

Mientras no se determine experimentalmente dicho umbral, para volúmenes
similares al evaluado se mantendría una solución basada en pandas o consultas
SQL, debido a que T1 no presentó una ventaja de rendimiento mediante Spark.
...
Se utilizó ChatGPT, de OpenAI, como herramienta de apoyo durante la elaboración
de este informe. Su uso estuvo orientado a la revisión de la redacción técnica,
organización del contenido según la rúbrica, comprobación de cálculos
matemáticos y apoyo en la estructuración del documento en \LaTeX.

La herramienta fue utilizada principalmente en las secciones de análisis de
speedup, contraste con la Ley de Amdahl, diagnóstico mediante Karp-Flatt,
propuesta de integración al PFC, dimensionamiento y revisión de conclusiones.

Los datos experimentales no fueron generados ni modificados mediante
inteligencia artificial. Los tiempos analizados proceden de las ejecuciones
registradas en el repositorio PE-U4 del Equipo C, incluidas las mediciones
adicionales de T1 con uno y dos executors.

La interpretación final de los resultados, la selección de la transformación
T1 y las decisiones relacionadas con la propuesta para Tienda Tech fueron
revisadas y asumidas por el autor del informe.
